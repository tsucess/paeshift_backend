...
# ==
# ✅ JOB NOTIFICATION CONSUMER
# ==
class JobNotificationConsumer(BaseWebSocketConsumer):
    """Handles real-time job-related notifications."""

    async def connect(self):
        self.group_name = f"user_job_notifications_{self.scope['user'].id}"
        await super().connect()

    async def job_notification(self, event):
        """Sends job-related notifications."""
        await self.send(text_data=json.dumps(event))


# ==
# ✅ REVIEW NOTIFICATION CONSUMER
# ==
class ReviewNotificationConsumer(BaseWebSocketConsumer):
    """Handles real-time review notifications."""

    async def connect(self):
        self.group_name = f"user_review_notifications_{self.scope['user'].id}"
        await super().connect()
...
# ==
# ✅ DISPUTE NOTIFICATION CONSUMER
# ==
class DisputeNotificationConsumer(BaseWebSocketConsumer):
    """Handles real-time dispute notifications."""

    async def connect(self):
        self.group_name = f"user_dispute_notifications_{self.scope['user'].id}"
        await super().connect()

    async def dispute_notification(self, event):
        """Sends dispute notifications."""
        await self.send(text_data=json.dumps(event))
...
# ==
class WalletNotificationConsumer(BaseWebSocketConsumer):
    """Handles real-time wallet/transaction notifications."""

    async def connect(self):
        self.group_name = f"user_wallet_notifications_{self.scope['user'].id}"
        await super().connect()

    async def wallet_notification(self, event):
        """Sends wallet/transaction notifications."""
        await self.send(text_data=json.dumps(event))
