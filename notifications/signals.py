...
def create_status_notification(user, entity_type, entity_title, old_status, new_status):
    ...
    status_messages = {
        ...
        'application': {
            'accepted': ("[SUCCESS] You're Hired!", f"Congrats, {user.get_full_name()}! Your application for '{entity_title}' was accepted. Check the job details to get started."),
        ...
        if created:
            ...
            if instance.location_coordinates:
                ...
                for user in nearby_users:
                    create_notification(
                        user=user,
                        title="[LOCATION] New Job Opportunity Nearby!",
    ...
    try:
        ...
        if instance.selected_applicant:
            create_notification(
                ...
                title="[ERROR] Job No Longer Available",
...
def handle_job_completion(sender, instance, created, **kwargs):
    # Only handle notifications when job transitions to COMPLETED (not business logic)
    if not created and instance.status == Job.Status.COMPLETED:
        # Send notifications about job completion
        # Note: Payment distribution and other business logic is handled in end_shift() method
        try:
            # Add any notification logic here if needed
            # For now, we'll just log the completion
            logger.info(f"Job {instance.id} has been completed")
        except Exception as e:
            logger.error(f"Error handling job completion notification: {str(e)}")
    ...
    try:
        if created:
            create_notification(
                ...
                title="[INFO] New Application Received",
            ...
            if instance.status == 'Rejected':
                create_notification(
                    user=instance.applicant,
                    title="[ERROR] Application Rejected",
            ...
            if instance.status == 'Accepted':
                create_notification(
                    user=instance.applicant,
                    title="[SUCCESS] Application Accepted",
    ...
    if created:
        try:
            create_notification(
                ...
                title="[INFO] Application Status Update",
    ...
    try:
        ...
        if created:
            ...
            if instance.recipient:
                create_notification(
                    ...
                    title="[PAYMENT] Payment on the Way",
        ...
        # Only send status change notification if status just changed to completed
            ...
            if instance.recipient:
                create_notification(
                    user=instance.recipient,
                    title="[PAYMENT] Payment Received",
...
def handle_escrow_updates(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            create_notification(
                user=instance.client,
                title="[ACCOUNT] Funds Secured in Escrow",
    ...
    try:
        ...
        if created:
            ...
            if instance.job.client != instance.created_by:
                create_notification(
                    user=instance.job.client,
                    title="[DISPUTE] New Dispute Filed",
            ...
            if instance.assigned_admin and instance.status == 'assigned':
                create_notification(
                    ...
                    title="[DISPUTE] Dispute Assigned to You",
            ...
            if instance.status == 'resolved' and instance.resolution_notes:
                for user in [instance.created_by, instance.job.client]:
                    if user != instance.created_by or instance.job.client != instance.created_by:
                        create_notification(
                            user=user,
                            title="[DISPUTE] Dispute Resolution Details",
    ...
    try:
        if created:
            ...
            create_notification(
                ...
                title="[REVIEW] New Review Received",
    ...
    try:
        if created:
            ...
            for admin_id in admins:
                create_notification(
                    user_id=admin_id,
                    title="[INFO] New Subcategory Added",
...
def handle_notification_preference_update(sender, instance, created, **kwargs):
    ...
    try:
        if not created:
            create_notification(
                user=instance.user,
                title="[INFO] Notification Preferences Updated",
...
#             create_notification(
#                 user=instance.user,
#                 title="[SUCCESS] Achievement Unlocked!",
