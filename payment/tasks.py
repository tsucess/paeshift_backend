...
def process_payment_webhook(reference, payment_method, status, raw_data):
    ...
    """
    from payment.models import Payment, PaymentStatusEnum
    ...
    if payment.status in [PaymentStatusEnum.SUCCESSFUL, PaymentStatusEnum.FAILED]:
        logger.info(f"Payment {reference} already finalized: {payment.status}")
        return {"status": "skipped", "message": "Already finalized"}
    ...
    with transaction.atomic():
        if status.lower() == "success" and verification_result.get("status") == "success":
            ...
            Notification.objects.create(
                user=payment.payer,
                message=f"Your payment of {payment.original_amount} failed.",
                category="payment",
            )
...
def verify_payment_status(self, reference):
    ...
    if payment.status != PaymentStatusEnum.SUCCESSFUL:
        try:
            payment.mark_as_successful()  # triggers FSM + updates job
            payment.verified_at = timezone.now()
            payment.save(update_fields=["status", "verified_at"])
        ...
        logger.info(f"ℹ️ Payment already marked successful: {reference}")
