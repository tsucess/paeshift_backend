...
def get_wallet_balance(user) -> Decimal:
    """Return the wallet balance for a given user, or 0.00 if no wallet exists."""
    try:
...
def initialize_payment(amount: Decimal, email: str, callback_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Payment init successful: tx_ref={tx_ref}, link={result.get('data', {}).get('link')}"
        )
        return result
...
def paystack_transfer(bank_details: dict, amount: Decimal) -> dict:
    """
    ...
    try:
        recipient_resp = requests.post(
            recipient_url, json=recipient_payload, headers=headers, timeout=15
        )
        ...
        if not recipient_data.get("status"):
            ...
            return {
                "status": "failed",
                "message": recipient_data.get("message", "Recipient creation failed."),
            }
    ...
    try:
        transfer_resp = requests.post(
            transfer_url, json=transfer_payload, headers=headers, timeout=15
        )
...
def flutterwave_transfer(bank_details: dict, amount: Decimal) -> dict:
    """
